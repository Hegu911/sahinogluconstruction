    ...
    <section ref={ref} className="py-14 lg:py-20 bg-surface">
      <div className="max-w-[1200px] mx-auto px-5 sm:px-8 lg:px-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-6">
          {images.map((img, i) => (
            <div key={img.alt} className={`overflow-hidden card-shadow rounded-xl transition-opacity duration-300 ${visible ? "opacity-100" : "opacity-0"}`} style={{ transitionDelay: `${i * 120}ms` }}>
              <img src={img.src} alt={img.alt} className="w-full aspect-[4/3] object-cover" />
              <div className="p-4 bg-surface">
                <span className="text-[0.6875rem] text-text-muted font-medium uppercase tracking-[0.06em]">{img.alt}</span>
              </div>
            </div>
          ))}
        </div>
