...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-surface">
      <aside className="w-48 bg-surface-alt shrink-0 hidden lg:flex flex-col">
        ...
        <nav className="p-3 flex-1">
          {links.map((link) => {
            ...
            return <Link key={link.href} href={link.href} className={`block px-3 py-2 text-[0.8125rem] rounded-[1px] mb-0.5 transition-colors ${active ? "bg-accent/15 text-accent-light" : "text-text-muted hover:text-text-primary hover:bg-surface-alt"}`}>{link.label}</Link>;
          })}
        </nav>
        <div className="p-3 border-t border-surface-alt">
