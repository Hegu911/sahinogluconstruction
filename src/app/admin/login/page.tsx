...
export default function AdminLoginPage() {
  ...
  return (
    <div className="min-h-screen bg-surface-lighter flex items-center justify-center px-5">
      <div className="w-full max-w-xs">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input type="text" value={username} onChange={(e) => setUsername(e.target.value)} className="w-full px-3 py-2.5 bg-surface-light border border-surface-lighter text-text-primary text-[0.8125rem] rounded-[1px] focus:outline-none focus:border-accent" required />
          </div>
          <div>
            <label className="block text-[0.6rem] font-medium tracking-[0.08em] uppercase text-text-muted mb-1.5">Şifrə</label>
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-3 py-2.5 bg-surface-light border border-surface-lighter text-text-primary text-[0.8125rem] rounded-[1px] focus:outline-none focus:border-accent" required />
          </div>
