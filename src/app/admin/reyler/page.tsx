  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="heading-lg text-text-primary">Rəylər</h1><p className="body-sm mt-0.5">{reviews.length} rəy</p></div>
      ...
      <div className="space-y-0">
        {reviews.map((r, i) => (
          <div key={`${r.name}-${i}`} className="flex items-start gap-3 p-3 bg-surface-light border-b border-border">
            <img src={r.image} alt={r.name} className="w-8 h-8 rounded-full object-cover shrink-0" />
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1.5 mb-0.5"><span className="text-[0.8125rem] font-medium text-text-primary">{r.name}</span><span className="text-[0.65rem] text-text-muted">· {r.projectTitle}</span></div>
