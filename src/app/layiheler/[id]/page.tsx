      ...
      <main className="pt-16 lg:pt-20">
        <section className="relative h-[35vh] sm:h-[40vh] lg:h-[45vh] min-h-[240px]">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-5 sm:p-8 lg:p-10 max-w-[1400px] mx-auto">
            <nav className="flex items-center gap-1.5 text-[0.6rem] text-paper/30 mb-3">
              <Link href="/" className="hover:text-paper transition-colors">Ana Səhifə</Link>
              <span>/</span>
              <Link href="/layiheler" className="hover:text-paper transition-colors">Layihələr</Link>
              <span>/</span>
          ...
          <div className="max-w-[1400px] mx-auto px-5 sm:px-8 lg:px-10">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-14">
              <div className="lg:col-span-8">
                <p className="body-sm mb-8 max-w-[520px]">{project.description}</p>
                <div className="editorial-rule mb-8" />

                <h3 className="heading-md text-ink mb-4">Qalereya</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-[1px] bg-border mb-10">
                  {project.gallery.map((img, i) => (
                    <div key={i} className="aspect-[4/3] bg-paper overflow-hidden">
                ...
                <h3 className="heading-md text-ink mb-4">Əvvəl / Sonra</h3>
                <div className="grid grid-cols-2 gap-[1px] bg-border">
                  <div className="aspect-[4/3] bg-paper relative overflow-hidden">
                    ...
                    <span className="absolute top-3 left-3 px-2 py-0.5 bg-ink/70 text-[0.5rem] font-medium tracking-[0.08em] uppercase text-paper rounded-[1px]">Əvvəl</span>
                  </div>
                  <div className="aspect-[4/3] bg-paper relative overflow-hidden">
                    <img src={project.afterImage} alt="Sonra" className="w-full h-full object-cover" />
                    <span className="absolute top-3 left-3 px-2 py-0.5 bg-ink/70 text-[0.5rem] font-medium tracking-[0.08em] uppercase text-paper rounded-[1px]">Sonra</span>
                  </div>
                </div>
              </div>
                    ...
                    <div className="space-y-0">
                      {[
                        ...
                        <div key={item.label} className="flex items-center justify-between py-2.5 border-b border-border">
                          ...
                          <span className="text-[0.8125rem] font-medium text-ink">{item.value}</span>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            {otherProjects.length > 0 && (
              <div className="mt-14 lg:mt-20 pt-8 lg:pt-10 border-t border-border">
                <h3 className="heading-md text-ink mb-5">Oxşar layihələr</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-[1px] bg-border">
                  {otherProjects.map((p) => (
                    <Link key={p.id} href={`/layiheler/${p.id}`} className="group flex items-center gap-3 p-3 lg:p-4 bg-paper hover:bg-paper-dark transition-colors">
                      <img src={p.heroImage} alt={p.title} className="w-16 h-12 object-cover rounded-[1px] shrink-0" />
                      <div className="flex-1 min-w-0">
                        <div className="text-[0.8125rem] font-medium text-ink group-hover:text-bronze transition-colors truncate">{p.title}</div>
