...
export default function ServiceDetailPage({
  ...
  const relatedProjects = PROJECTS.filter(
    (p) => p.category === service.slug || p.category === "ferdi-ev" || p.category === "villa" || p.category === "renovasiya"
  ).slice(0, 2);
        ...
        <section className="py-10 lg:py-20">
          <div className="max-w-[1400px] mx-auto px-5 sm:px-8 lg:px-10">
            <nav className="flex items-center gap-1.5 text-[0.65rem] text-text-muted mb-8 lg:mb-12">
              <Link href="/" className="hover:text-ink transition-colors">Ana Səhifə</Link>
              <span>/</span>
              <Link href="/xidmetler" className="hover:text-ink transition-colors">Xidmətlər</Link>
              <span>/</span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-14">
              <div className="lg:col-span-7">
                ...
                <h1 className="heading-xl text-ink mb-6">{service.title}</h1>
                <p className="body-sm mb-8 max-w-[520px]">{service.fullDescription}</p>

                <div className="editorial-rule mb-8" />

                <h3 className="heading-md text-ink mb-4">Xidmətə daxildir</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-8">
                  {service.features.map((f) => (
                    <div key={f} className="flex items-center gap-2 py-2.5 border-b border-border">
                  ...
                  <div className="py-6 border-t border-border">
                    ...
                    <div className="space-y-3">
                      <div className="flex items-start gap-2.5">
                        ...
                        <div>
                          <div className="text-[0.8125rem] font-medium text-ink">10 il struktur zəmanəti</div>
                      ...
                      <div className="flex items-start gap-2.5">
                        ...
                        <div>
                          <div className="text-[0.8125rem] font-medium text-ink">3 il tam təmir zəmanəti</div>
                      ...
                      <div className="space-y-3">
                        {relatedProjects.map((p) => (
                          <Link key={p.id} href={`/layiheler/${p.id}`} className="flex items-center gap-3 group">
                            ...
                            <div>
                              <div className="text-[0.8125rem] font-medium text-ink group-hover:text-bronze transition-colors">{p.title}</div>
